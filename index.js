...
app.get("/products", async (req, res) => {
  try {
    const [rows] = await db.query(
      "select id, sku, nama, kategori, satuan, stok_saat_ini, stok_minimum from products",
    );
    res.json(rows);
  } catch (err) {
...
function ambilRetryDelay(err) {
  // Coba baca saran waktu tunggu dari Gemini sendiri, kalau ada
  try {
...
app.post("/ai-insights/generate", verifyToken, async (req, res) => {
  try {
    const [produk] = await db.query(
...
app.post("/purchase-orders", verifyToken, async (req, res) => {
...
app.put("/purchase-orders/:id/selesai", verifyToken, async (req, res) => {
